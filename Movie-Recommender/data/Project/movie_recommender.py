...
def fetch_movies_for_emotion(emotion, max_per_genre=3):
    ...
    Returns list of movie dicts with keys: title, overview, poster, release_date, tmdb_id
    """
    if not TMDB_API_KEY:
...
def fetch_tmdb_movies_by_ids(ids):
    """
    Given a list of TMDB ids (ints or strings), fetch movie details for each and return
